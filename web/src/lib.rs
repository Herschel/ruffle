    ...
    pub fn destroy(&mut self) {
        ...
        if let Ok(mut instance) = self.remove_instance() {
            ...
            if let Some(id) = instance.animation_handler_id {
                if let Some(window) = web_sys::window() {
                    window.cancel_animation_frame(id.into()).warn_on_error();
                }
            }
        }
...
impl Ruffle {
    ...
    fn tick(&mut self, timestamp: f64) {
        ...
        let (canvas, dt) = if let Ok(instance_data) = self.with_instance_mut(|instance| {
        ...
        let new_dimensions = self
            .with_instance_mut(|instance| {
                    ...
                    || (instance.device_pixel_ratio - device_pixel_ratio).abs() >= f64::EPSILON
                {
                    ...
                    Some((viewport_width, viewport_height, device_pixel_ratio))
                } else {
                    None
                }
            })
