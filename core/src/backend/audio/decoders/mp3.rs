...
pub mod symphonia {
    ...
    // FIXME: symphonia currently doesn't require `Send` for its `MediaSource` or `Decoder` structs,
    // which prevents us from sending our decoder to another thread. This should be safe, so hack it in for now.
    // Remove when symphonia adds `Send` to its trait bounds.
    unsafe impl Send for Mp3Decoder {}

    #[allow(dead_code)]
    pub struct Mp3Decoder {
        reader: symphonia_bundle_mp3::Mp3Reader,
    ...
    impl Mp3Decoder {
        pub fn new<R: 'static + Read>(num_channels: u16, sample_rate: u32, reader: R) -> Self {
            ...
            let reader =
                symphonia_bundle_mp3::Mp3Reader::try_new(source, &Default::default()).unwrap();
        ...
        pub fn new_seekable<R: 'static + AsRef<[u8]>>(
            num_channels: u16,
            sample_rate: u32,
            reader: Cursor<R>,
        ) -> Self {
    ...
    impl SeekableDecoder for Mp3Decoder {
        ...
        fn reset(&mut self) {
            let _ = self
                .reader
                .seek(formats::SeekTo::TimeStamp { stream: 0, ts: 0 });
