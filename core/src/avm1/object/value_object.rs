    ...
    pub fn boxed(activation: &mut Activation<'_, 'gc, '_>, value: Value<'gc>) -> Object<'gc> {
        if let Value::Object(ob) = value {
            ...
            let proto = match &value {
                Value::Bool(_) => Some(activation.context.gc_data.avm1.prototypes.boolean),
                Value::Number(_) => Some(activation.context.gc_data.avm1.prototypes.number),
                Value::String(_) => Some(activation.context.gc_data.avm1.prototypes.string),
                _ => None,
            };
